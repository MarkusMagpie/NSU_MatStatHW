...
n = len(data)
k = int(1 + 3.322 * np.log10(n)) # число интервалов
...
if (res >= c_kvantil):
    print(f"отвергаем H_0: данные НЕ следуют равномерному распределению U[0, 1] ({res:.3f} >= {c_kvantil:.3f})")
else:
    print(f"принимаем H_0: данные согласуются с равномерным распределением U[0, 1] ({res:.3f} < {c_kvantil:.3f})")

print(f"\nРДУЗ (p-value): {p_value:.3f}")
